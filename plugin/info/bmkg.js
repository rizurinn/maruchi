const cuaca = {
    ...
    getkWeatherByCoordinateBMKG: async function (latitude, longitude, placeName = "") {
        ...
        const namaTempat = placeName.trim().length ? "*🗾 Daerah:* " + placeName + '\n' : ''
        ...
        const angin = `Angin bertiup dari ${arahAngin[wd]} ke ${arahAngin[wd_to]} dengan kecepatan ${ws} km/h. Sudut arah ${wd_deg}°`
        
        const cuaca =  "*📍 Lokasi:* " + lokasi + "\n" +
            "*🕛 Jam:* " + local_datetime.split(" ")[1] + " (Waktu Setempat)\n" +
            "*🌤️ Cuaca:* " + weather_desc + "/" + weather_desc_en + "\n" +
            "*🌡️ Suhu:* " + t + "°C\n" +
            "*💦 Kelembaban:* " + hu + "%\n" +
            "*☁️ Tutupan awan* " + tcc + "%\n" +
            "*👀 Jarak pandang:* " + vs_text + " (" + vs + " m)" + "\n" +
            "*🌬️ Angin:* " + angin
        ...
        const cuacaWarning = "*📰 Dampak:* " + dampak + "\n" +
            "*🚨 Peringatan:* " + peringatan

        const bmkgUrl = "*🌐 BMKG Link:* " + `https://www.bmkg.go.id/cuaca/prakiraan-cuaca/${adm4}`
        const gmapUrl = "*🗺️ Google Maps:* " + `https://www.google.com/maps?q=${latitude},${longitude}`

        const final = "☁️ *Prakiraan Cuaca* ☁️\n\n" + namaTempat + cuaca + '\n\n' + cuacaWarning + '\n\n' + bmkgUrl + '\n' + gmapUrl
        return final
    },
...
${usedPrefix + command} gempa - Informasi gempa terkini`)
  ...
  try {
    await loading()
    if (tipe === 'cuaca') {
      if (!args[1]) return m.reply(`🍭 *Masukkan nama daerah*\n*Contoh:* ${usedPrefix + command} cuaca sabang`)
    ...
    return m.reply('🍰 *Pilihan hanya: cuaca / gempa*')
